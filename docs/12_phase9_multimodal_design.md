...
#### 3.3.1 图片 base64 流量量化（R13 第三轮核实新增）

> ⚠️ **R13 第三轮 web 核实新增**（[R13](./14_feasibility_recheck_and_plan.md)）：原设计未量化 base64 流量，给用户预估联网成本不足。

**流量估算**（不同压缩档位下单张图片 base64 后体积）：

| 档位 | 原图尺寸 | base64 后体积 | 单次对话流量（5 轮多轮） | 备注 |
|---|---|---|---|---|
| 原图 | 1080p (2-4MB) | **2.7-5.3MB** | 13.5-26.5MB | 视觉 API 上传 + 下载（响应文本）流量 |
| 高压缩（推荐） | 720p JPEG q70 (200-400KB) | **270-540KB** | 1.35-2.7MB | 推荐 MVP 默认 |
| 中压缩 | 540p JPEG q60 (100-200KB) | **135-270KB** | 0.7-1.4MB | 极省流量，但可能影响识别精度 |

**关键观察**：
- base64 编码本身膨胀 33%（每 3 字节 → 4 字符）
- 多轮对话上下文超 5 轮后丢弃最早图，但当前轮仍需带 1 张图 + 5 轮文本 → 单次请求 payload ~2MB（高压缩档）
- 4G 网络下高压缩档单次请求延迟可能 5-15s（上行带宽受限）
- **超时风险**：原图 + 多轮易触发 HTTP client 30s 超时（[§11 风险表](#11-风险与缓解) 已列）

**推荐策略**：
- MVP 默认"高压缩"档位（720p JPEG q70）
- 在 UI 配置页加流量警示："每张图片大约消耗 0.3-0.5MB 流量，多轮对话累计可达数 MB"
- 原图档位需用户主动选择 + 二次确认"原图上传可能消耗数 MB 流量且较慢"
- 移动数据网络下自动降级到"中压缩"档（可选开关，默认关）
...
### 4.1 GLM-4V API 实名认证与配额（R12 第三轮核实新增）

> ⚠️ **R12 第三轮 web 核实新增**（[R12](./14_feasibility_recheck_and_plan.md)）：原设计仅写"用户需配置 vision API key"，未说明 GLM-4V API 的实名认证门槛与配额限制，用户实际开通时可能踩坑。

**GLM-4V API 开通门槛**（智谱开放平台 `open.bigmodel.cn`）：
1. **手机号 + 实名认证**：智谱开放平台强制要求个人开发者完成手机号 + 中国大陆身份证实名认证后才可调用 GLM-4V 接口
2. **企业认证**：可选，企业认证后可解锁更高 QPS 配额（个人默认 QPS 较低，多轮对话密集调用时可能被限流）
3. **新用户免费额度**：智谱对新注册用户有赠送 token，但 GLM-4V 多模态调用单次成本高于纯文本，免费额度消耗快
4. **境外用户**：境外身份证 + 海外手机号用户无法直接实名，需要走企业认证或更换 API
5. **API key 安全**：智谱 API key 一旦泄漏，他人可消耗用户余额；MiBrain 在 Settings 里存储 API key 时应使用 Android Keystore 加密，不要明文存 SharedPreferences

**对 MiBrain 的影响**：
- Phase 9 实施时，UI 首次启用看图对话必须强制弹出实名认证说明："GLM-4V 需到智谱开放平台 `open.bigmodel.cn` 完成实名认证后获取 API key，境外用户请改用其他 API"
- 隐私边界（[§5](#5-隐私边界)）补充：实名认证会将用户真实身份与 API 调用绑定，看图对话内容理论上可被智谱关联到实名身份
- 备选 API：GPT-4V（境外可付费，但需 OpenAI 账号 + 国内访问需代理）/ Claude 3.5 Sonnet（同上）/ Qwen-VL（阿里云需实名）/ 本地多模态（如 Qwen-VL 本地版，但 8GB RAM 设备内存压力极大，**8GB 不可用**）
