...
| Cap | 做什么 | 主要受众 |
|---|---|---|
| Cap 1: 本地 API 暴露 | 把 llama-server 的 8080 端口产品化，加 UI 配置 + 限流 + token 计数 | 同机开发者、Tasker/MacroDroid 用户 |
...
llama-server 已在 127.0.0.1:8080 提供 OpenAI 兼容 API（Phase 1 即有）。但当前是裸的——没有 UI 配置、没有限流、没有日志。

### 1.2 Phase 8 增强项

| 项 | 实现 | 默认值 |
|---|---|---|
| 端口配置 | Settings → 平台 → API 端口 | 8080 |
...
通过 llama-server 的 `usage` 字段（OpenAI 协议自带），每次响应记录：
- input_tokens
- output_tokens
- 模型名
- 调用来源 IP（如果是 LAN 暴露）
...
| MCP 工具名 | 做什么 | 实际调用 |
|---|---|---|
| `mibrain.chat` | 调本地 LLM 推理 | llama-server 8080 |
...
| llama-server 限流不生效导致 OOM | 中 | 中 | 单 IP 限流 + 全局 token 上限 |
...
- **Cap 1** 不依赖 Phase 6/7（llama-server 本身就是 Phase 1 基础设施）
