...
- APK 体积增加 ~50MB（含 arm64-v8a 的 `libllama.so` ~20MB + `libggml.so` ~10MB + `libc++_shared.so` ~1MB + `libsherpa-onnx-jni.so` 3.7MB + `libonnxruntime.so` 15MB，实测数据来自日本 Pasona 部署实录；原"合计 30MB"仅含 libllama+libggml，遗漏 sherpa-onnx AAR 的两个 .so）
...
> **第六轮 R4 扩展**：原接口仅 5 个方法（loadModel/complete/streamComplete/unloadModel/getVersion），缺 system prompt / 多轮上下文 / sampler 参数 / function calling / 线程安全 / 多 profile 隔离共 9 项。本节扩展为分 Phase 渐进添加的完整接口。

```kotlin
// === LlamaConfig：采样参数（Phase 1 起可用，解决 R4 "top_p/top_k/stop sequences 未暴露"） ===
data class LlamaConfig(
    val nCtx: Int = 2048,            // 上下文窗口
    val nBatch: Int = 512,
    val nThreads: Int = 2,
    val temperature: Float = 0.7f,
    val topP: Float = 0.9f,          // R4 补：top-p 采样
    val topK: Int = 40,              // R4 补：top-k 采样
    val repeatPenalty: Float = 1.1f, // R4 补：重复惩罚
    val stopSequences: List<String> = listOf(),  // R4 补：停止序列（如 ["</tool>"]）
    val systemPrompt: String = "",   // R4 补：系统提示（Phase 1 可空，Phase 6 起用 tool-use prompt）
)
...
class LlamaEngine(private val modelPath: String, private val config: LlamaConfig) {
    ...
    // === R4 扩展：Phase 1 起可用（上下文管理，解决"多轮上下文/clearContext"缺失） ===
    // 注意：b9830 KV cache 由 llama_memory_t 承载（[15 §3.1](./15_technical_specs.md)）
    external fun setSystemPrompt(prompt: String)        // R4 #8：system prompt
    external fun setHistory(history: Array<ChatMessage>) // R4 #2/#9：多轮上下文（重写 KV cache）
    external fun clearContext()                          // R4 #3：清空 KV cache（llama_memory_clear）
    external fun saveContext(sessionPath: String): Boolean  // R4：持久化 KV cache 到文件（[03 §3.4](#34-keep-alive-卸载策略) keep-alive 前保存）
    external fun restoreContext(sessionPath: String): Boolean  // R4：恢复（重新加载后接续多轮）
    external fun setSamplerParams(                       // R4 #7：运行时改采样参数（top_p/top_k/stop）
        temperature: Float, topP: Float, topK: Int,
        repeatPenalty: Float, stopSequences: Array<String>
    )

    // === R4 扩展：Phase 6 起可用（function calling，解决"completeWithTools"缺失） ===
    // Qwen2.5-1.5B 无原生 function calling（[D25](../DECISIONS.md)），用 32-token CoT prompt 模拟
    external fun completeWithTools(
        prompt: String,
        tools: Array<ToolSpec>,        // 工具描述（JSON schema）
        maxTokens: Int,
        callback: (token: String) -> Unit
    ): String  // 返回值含 <tool_call> 标签或纯文本，由 ToolRouter 解析

    // === R4 扩展：多 profile 隔离（Phase 10，每个 profile 独立 KV cache + system prompt） ===
    external fun switchProfile(profileId: String): Boolean  // R4 #5：切 profile（unload 当前 → load 新）

    companion object {
        init { System.loadLibrary("llama_engine") }
    }
}

data class ChatMessage(val role: String, val content: String)  // role: "user"/"assistant"/"system"
data class ToolSpec(val name: String, val description: String, val parameters: String)  // parameters: JSON schema
...
**线程安全约束（R4 #4，必须遵守）**：
- `streamComplete` / `completeWithTools` 在**单一推理线程**执行（[15 §2](./15_technical_specs.md) JNI callback 跨线程规范）
- `setSystemPrompt` / `setHistory` / `clearContext` / `setSamplerParams` / `switchProfile` **不得在推理进行中调用**（否则竞态改 KV cache 导致崩溃）；ConversationEngine 用 `inferenceLock`（[15 §7.4 fix #4](./15_technical_specs.md)）保证这些调用只在非 THINKING 态触发
- `saveContext` / `restoreContext` 是 I/O 操作，可异步，但需与 `unloadModel` 互斥（keep-alive 卸载前必须先 saveContext）

**Phase 渐进添加顺序**：
| 方法 | 引入 Phase | 依赖 |
|---|---|---|
| loadModel / complete / unloadModel / getVersion | Phase 1 | [15 §1 CMake](./15_technical_specs.md) + §3 C API |
| streamComplete | Phase 2 | [15 §2 JNI callback](./15_technical_specs.md) |
| setSystemPrompt / setHistory / clearContext / setSamplerParams / saveContext / restoreContext | Phase 2（多轮对话） | [15 §3.1 llama_memory_*](./15_technical_specs.md) |
| completeWithTools | Phase 6 | [D17](../DECISIONS.md) 关键词正则 + LLM 兜底 + [D25](../DECISIONS.md) 32-token CoT |
| switchProfile | Phase 10 | [13 §7](./13_phase10_ux_enhancements_design.md) 多用户 |
...
| LSPosed Vector runtime | 100MB | 100MB | Vector v2.0（2026-03-22 发布，[F1](./14_feasibility_recheck_and_plan.md)；第六轮 R5 订正：公开 release 仅此 tag，原 v2.0.3-7716 无法对应） |
...
**对比 3B 模型（~~质量优先模式~~ 已废弃——第三轮重算后 8GB 设备必 OOM，[D30](../DECISIONS.md)）**：
...
- 两者之和 60MB，加上 onnxruntime 推理引擎常驻 ~15MB（`libonnxruntime.so`，已计入 §6 内存预算的 sherpa-onnx .so 19MB）= 79MB，**无法通过"串行执行"释放**（原"onnxruntime 80MB"为高估，实际 .so 仅 15MB）

如果系统压力上来，APK 监听 `onTrimMemory(TRIM_MEMORY_RUNNING_CRITICAL)` 主动调 `unloadModel()` 卸载 LLM 模型，下次唤醒时重新加载（接受 3-5s 加载延迟）。VAD + KWS 不卸载。

### 6.1 Phase 6-10 新增组件内存预算（第六轮 R4 补，解决"未来 Phase 增量未计入"遗留）

> 第六轮 R4 排查发现：原 §6 表仅覆盖 Phase 1-5（基础推理 + ASR/TTS/VAD/KWS）。Phase 6-10 新增组件的内存占用未计入，可能导致"Phase 6 启用联网工具后峰值超 7.89GB"。本节按 Phase 渐进补强，作为是否启用各 Phase 的内存门槛依据。

| Phase | 新增组件 | 常驻增量 | 峰值增量（最坏叠加） | 累计常驻 | 累计峰值 | 是否影响 8GB 门槛 |
|---|---|---|---|---|---|---|
| Phase 6 | NetworkGate + ToolRouter + 4 个 Tool.kt（仅 Kotlin 对象 + HTTP 客户端 OkHttp） | 30MB | 50MB（含 OkHttp 连接池 + TLS buffer） | 6.67GB | 7.94GB | 临界（峰值仅余 60MB headroom） |
| Phase 7 | NotificationListenerService + AppLauncherTool + SystemSettingsTool + ScreenCaptureTool | 20MB | 80MB（截屏 Bitmap 1080×1920×4 = ~8MB + ImageReader 双缓冲） | 6.69GB | 8.02GB | **超 8GB！** ScreenCaptureTool 启用时必 OOM，见下文缓解 |
| Phase 8 | 内嵌 HTTP server（Ktor/NanoHTTPD）+ MCP server + Widget | 40MB | 70MB（HTTP 连接池 + MCP 序列化 buffer） | 6.73GB | 8.09GB | **超 8GB！** Cap 1 启用时必须强制卸载 LLM |
| Phase 9 | PaddleOCR NCNN 模型 + GLM-4V 网络客户端 | - | 250MB（OCR 推理 200MB + Bitmap 缓存 50MB） | 6.73GB | 8.34GB | **超 8GB！** OCR 推理时必须卸载 LLM 模型 |
| Phase 10 | 多 profile 上下文 + 儿童模式 + 音频路由 + 全局暂停 | 50MB | 80MB（多 profile 各自 KV cache 切换 buffer） | 6.78GB | 8.42GB | **超 8GB！** 切 profile 时必须强制 unload 当前 LLM |

**结论**：
- **Phase 1-5（MVP）**：峰值 7.89GB，headroom 0.11GB，紧张但可接受（[03 §6](#6-内存预算2026-06-30-第三轮严格重算修正算术错误-e10) 原结论不变）
- **Phase 6 联网工具**：峰值 7.94GB，headroom 仅 60MB，**临界**。缓解：联网工具调用期间禁用 ScreenCaptureTool / OCR / MCP（本 Phase 7+ 才有，自然不冲突）
- **Phase 7 手机控制**：峰值 8.02GB，**超 8GB 限**。缓解：ScreenCaptureTool 执行时强制 `unloadModel()` 释放 1.3GB，截屏完成后重新 `loadModel()`（接受 3-5s 重新加载延迟）；与 [03 §3.4 keep-alive](#34-keep-alive-卸载策略) 配合
- **Phase 8 平台化**：峰值 8.09GB，**超 8GB 限**。缓解：Cap 1 启用本地 API 暴露时必须强制卸载 LLM 模型（API server 单独常驻 ~40MB，LLM 仅在推理请求时按需 load）
- **Phase 9 多模态**：峰值 8.34GB，**超 8GB 限**。缓解：OCR 推理期间强制 `unloadModel()` 释放 1.3GB（OCR ~200MB + LLM 1.3GB = 1.5GB，但 unload 后峰值降至 7.04GB，余 1GB headroom）
- **Phase 10 UX 增强**：峰值 8.42GB，**超 8GB 限**。缓解：多 profile 切换时强制 unload 当前 LLM → load 新 profile（已由 [03 §3.1](#31-接口签名草案) `switchProfile()` 内部实现保证）

**8GB 设备 Phase 启用门槛（硬规则）**：
| Phase | 是否可在 8GB 设备启用 | 强制条件 |
|---|---|---|
| 1-5 | ✅ 可 | - |
| 6 | ✅ 可（临界） | 联网工具调用时 ScreenCaptureTool / OCR / MCP 必须处于未启用状态 |
| 7 | ⚠️ 仅条件启用 | ScreenCaptureTool 必须先 `unloadModel()`，截屏后重新加载 |
| 8 | ⚠️ 仅条件启用 | Cap 1 启用时 LLM 改为按需 load（请求来时 load，超时 unload） |
| 9 | ⚠️ 仅条件启用 | OCR 推理期间必须 `unloadModel()`，OCR 完成后 reload |
| 10 | ⚠️ 仅条件启用 | `switchProfile()` 内部必须先 unload 当前 → load 新 profile |

**12GB+ 设备**：以上 Phase 全部可同时启用，headroom ≥ 3GB。

**关联**：本节是 [D30](../DECISIONS.md)（3B 模型 8GB 不可用）的延续——3B 模型即使 Phase 1-5 已必 OOM，Phase 6+ 更无空间，**12GB+ 设备或 Phase 11+ Adreno GPU 加速前** 3B 永久不可用。

**反向引用（第六轮 R4 补强）**：本节"ScreenCaptureTool / OCR / MCP / switchProfile 启用时必须先 `unloadModel()`"的强制条件落地依赖 [15 §6.5](./15_technical_specs.md)（资源泄漏防护清单，资源 #7 ONNX session + LLM 模型互斥释放，unload 顺序须遵循）+ [15 §7.4](./15_technical_specs.md)（watchdog 强制 reset 路径，watchdog 检测到卡死态时强制 `unloadModel()`，防止 unload 与推理线程竞态）。两者共同保证"先 unload 再启高内存 Phase"的原子性。
...
- TTS 改 vits-zh-ll（社区贡献，许可未明确声明；~~发布前需替换为 matcha-icefall-zh-baker~~ **第四轮 [F7](./14_feasibility_recheck_and_plan.md) 确认 matcha-icefall-zh-baker 受 Data-Baker NC 限制同样不可分发**，发布前须另寻 Apache 2.0 中文 TTS 或自训）
