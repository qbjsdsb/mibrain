...
- APK 体积增加 ~50MB（含 arm64-v8a 的 libllama.so + libggml.so + sherpa-onnx .so）
...
| TTS 模型 | `${model_dir}/sherpa-vits-zh-ll/` | Apache 2.0（替代 aishell3） |
...
## 6. 内存预算（2026-06-30 重新核算，解决 S4）

8GB 总内存，默认 1.5B 模型：

| 组件 | 常驻内存 | 峰值内存 | 备注 |
|---|---|---|---|
| HyperOS 2 + 系统服务 | 5.0GB | 5.5GB | 不可压缩（实测可能 5.5-6GB） |
| LSPosed runtime | 100MB | 100MB | |
| Phantom Mic (LSPosed) | 30MB | 30MB | |
| MiBrain APK 基础 | 100MB | 100MB | Compose + AndroidX |
| libllama.so + libggml.so | 50MB | 50MB | JNI 库本身 |
| sherpa-onnx .so + onnxruntime | 80MB | 80MB | 一个 AAR 覆盖 ASR/TTS/VAD/KWS |
| **Qwen2.5-1.5B Q4_K_M** | **0.9GB** | **1.2GB** | **含 KV cache 2048 ctx**（默认） |
...
| **常驻合计（1.5B 默认）** | **6.32GB** | - | 留 1.68GB headroom |
| **峰值合计（1.5B 默认）** | - | **6.77GB** | 串行执行 ASR→LLM→TTS |

**结论（默认 1.5B）**：峰值 6.77GB，留 1.23GB headroom，**安全**。

**对比 3B 模型（质量优先模式）**：

| 组件 | 3B 常驻 | 3B 峰值 |
|---|---|---|
| Qwen2.5-3B Q4_K_M | 1.8GB | 2.2GB |
| **常驻合计（3B）** | **7.22GB** | - |
| **峰值合计（3B）** | - | **7.67GB** |

3B 模型峰值 7.67GB，仅留 330MB headroom，**风险高**。Phase 4 真机测试时实测，若 OOM 则禁用 3B 选项。
...
- TTS 改 Apache 2.0 许可的 vits-zh-ll
...
- [ ] LlamaEngine.kt 的 C++ JNI wrapper 实现（参考 [llama.android](https://github.com/ggml-org/llama.cpp/tree/master/llama.android) 官方模块 + [ToolNeuron LlamaEngine.kt](https://github.com/Siddhesh2377/ToolNeuron)）
